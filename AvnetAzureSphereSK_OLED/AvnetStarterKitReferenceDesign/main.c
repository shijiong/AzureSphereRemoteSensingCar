...
// Provide local access to variables in azure_iot_utilities.h to control the car
...
static void UART0EventHandler()
{
	...
	// Read UART message
	bytesRead = read(uart0Fd, receiveBuffer, receiveBufferSize);
	if (bytesRead < 0) {
...
//Car Control EventHandler Shijiong
/// <summary>
///     Handle car control event.
/// </summary>
static void CarControlHandler()
{
	uint8_t commandSequence[5];

	if (moveLeft)
	{
		moveLeft = false;
		commandSequence[0] = 0xff;
		commandSequence[1] = 0x00;
		commandSequence[2] = 0x03;
		commandSequence[3] = 0x00;
...
static int InitPeripheralsAndHandlers(void)
{
	...
	uart0Fd = UART_Open(AVT_SK_CM1_ISU0_UART, &uartConfig);
	if (uart0Fd < 0) {
		Log_Debug("ERROR: Could not open UART: %s (%d).\n", strerror(errno), errno);
		return -1;
	}

	//car control variables shijiong
	moveLeft = false;
	moveRight = false;
	moveForward = false;
	moveBack = false;
...
static void ClosePeripheralsAndHandlers(void)
{
	...
	//Uart shijiong
	CloseFdAndPrintError(uart0Fd, "Uart");
